...
// Create a payment "session" (stub) for an invoice
app.post('/api/payments/session', ensureBuyerOrAdmin, async (req, res) => {
  try {
    ...
    const inv = await db.query(`SELECT i.*, o.buyer_id FROM public.invoices i JOIN public.orders o ON o.id=i.order_id WHERE i.id=$1`, [invoice_id]);
    ...
    const pay = await db.query(`
      INSERT INTO public.payments (invoice_id, provider, provider_session_id, status, amount_cents, currency, raw)
      VALUES ($1,$2,$3,'requires_action',$4,$5,$6)
      RETURNING *;
    `, [invoice_id, provider, sessionId, row.amount_cents, row.currency, { action: 'redirect', fake_url: `/pay/fake?sid=${sessionId}` }]);
...
// Simulated provider callback (webhook) to capture payment
