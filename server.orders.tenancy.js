...
app.get('/api/orders', ensureBuyerOrAdmin, async (req, res) => {
  try {
    ...
    if (tenant && !scopeAll) {
      args.push(tenant); wh.push(`(o.tenant_id = $${args.length})`);
    }
    const sql = `
      SELECT o.*, 
...
app.get('/api/orders/:id', ensureBuyerOrAdmin, async (req, res) => {
  try {
    ...
    const o = ord.rows[0];
    if ((o.buyer_id !== (actor.id||'')) && !hasRole(actor, 'admin')) return res.status(403).json({ error: 'forbidden' });
    if (tenant && o.tenant_id && tenant !== o.tenant_id && !hasRole(actor, 'admin')) return res.status(403).json({ error: 'wrong tenant' });
