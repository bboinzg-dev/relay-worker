...
const { getTenant } = require('./src/utils/tenant');

const app = express();
app.use(cors());
app.use(bodyParser.json({ limit: '10mb' }));

async function pickCheapestPlan(items, tenant){
  const assignments = [];
  for (const it of items) {
    ...
    const q = await db.query(`
      ...
      WHERE brand_norm=lower($1) AND code_norm=lower($2)
        AND status='approved'
        AND ($3::text IS NULL OR tenant_id IS NULL OR tenant_id=$3::text)
    ...
    if (remain > 0) {
      assignments.push({ brand, code, qty: remain, unit_price_cents: 0, currency: 'USD', listing_id: null, is_alternative: false, lead_time_days: null, backorder: true });
    }
  }
  let subtotal = 0; for (const a of assignments) subtotal += Number(a.unit_price_cents||0) * Number(a.qty||0);
  const tax = Math.round(subtotal * 0.0); const shipping = 0; const total = subtotal + tax + shipping;
...
app.post('/api/checkout/preview', ensureBuyer, async (req, res) => {
  try {
    ...
    const plan = await pickCheapestPlan(items, tenant);
...
app.post('/api/checkout/create', ensureBuyer, async (req, res) => {
  ...
  try {
    ...
    const tenant_id = actor.tenantId || null;
    const buyer_id = actor.id || null;
    const plan = req.body?.plan || await pickCheapestPlan(items, tenant_id);
    const totals = plan.totals;
    await client.query('BEGIN');
    await client.query('SELECT public.fn_set_actor($1,$2,$3::text[])', [buyer_id, tenant_id, (actor.roles||[])]);
    ...
    for (const a of plan.assignments) {
      await client.query(`
        INSERT INTO public.order_items
          (order_id, tenant_id, brand, code, brand_norm, code_norm, qty, unit_price_cents, currency, listing_id, is_alternative, lead_time_days)
        VALUES ($1,$2,$3,$4,lower($3),lower($4),$5,$6,$7,$8,$9,$10)
      `, [order.id, tenant_id, a.brand, a.code, a.qty, a.unit_price_cents, a.currency || 'USD', a.listing_id, !!a.is_alternative, a.lead_time_days]);
    }
    const inv = await client.query(`
      INSERT INTO public.invoices (order_id, tenant_id, invoice_no, status, currency, amount_cents)
      VALUES ($1,$2, 'I'||nextval('seq_invoice_no')::text, 'unpaid', $3, $4)
      RETURNING *;
    `, [order.id, tenant_id, order.currency || 'USD', order.total_cents]);
