...
const pool = new Pool({
  ...
  // Cloud SQL/Neon/Supabase 등 대부분 호환. 필요 시 PGSSLMODE=require 설정과 함께 사용.
...
async function tableExists(client, fqName /* 'public.users' */) {
  const { rows } = await client.query(`select to_regclass($1) as reg`, [fqName]);
  return !!rows[0]?.reg;
}

// users / user_profiles 테이블 보장 (없으면 생성)
// 운영 DB 권한이 없으면 CREATE는 무시(에러)될 수 있음
