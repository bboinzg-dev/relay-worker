...
function getModel(name) {
  const v = new VertexAI({ project: env.PROJECT_ID, location: env.VERTEX_LOCATION });
...
async function classifyFamily({ pdfBytes, allowedFamilies }) {
  const mdl = getModel(env.GEMINI_MODEL_CLASSIFY);
  const prompt = `
...
async function extractByBlueprint({ pdfBytes, family, fields }) {
  const mdl = getModel(env.GEMINI_MODEL_EXTRACT);
