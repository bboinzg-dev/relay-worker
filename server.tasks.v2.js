// Step 27 patch — include 'cover_render' event using pdf renderer
...
async function handleOne(ev){
  ...
  if (t === 'cover_render') {
    const brand = (p.brand||'').toString(); const code = (p.code||'').toString(); const gcsPdfUri = (p.gcsPdfUri||'').toString();
    const out = await renderCoverForPart({ gcsPdfUri, brand, code, outBucket: process.env.GCS_BUCKET });
...
app.post('/api/tasks/process-events-v2', async (req, res) => {
  try {
