...
try {
  app.use('/api', require('./src/routes/docs-requests'));
  console.log('[BOOT] mounted /api/docs-requests, /api/seller/docs-requests, /api/plan-bids');
} catch (err) {
  console.error('[BOOT] failed to mount docs-requests router:', err?.message || err);
}
...
async function ensureRelayPowerPnTemplate() {
  ...
  try {
    const res = await db.query(
      ...
      WHERE family_slug = 'relay_power'
        AND COALESCE(ingest_options->>'pn_template', '') <> $1`,
      [template]
    );
    if (res.rowCount > 0) {
      console.log('[BOOT] updated relay_power pn_template');
    }
  } catch (err) {
